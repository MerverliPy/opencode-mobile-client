...
5. If `next_action` is `next-phase`, execute the next-phase selection logic inline:
   - select the correct next phase
   - overwrite `.opencode/plans/current-phase.md`
   - rerun `bash scripts/dev/autoflow.sh inspect-json`
6. If `next_action` is `run-phase`, execute the active phase inline with builder discipline:
...
7. Immediately after implementation, execute validation inline with validator discipline:
...
8. If inline validation returns FAIL, allow one bounded fix pass only when the blockers are deterministic and narrow:
...
9. If `next_action` is `validate-phase`, run validation inline now instead of delegating.
10. If `next_action` is `fix-validation`, run the bounded validation-fix pass inline now instead of delegating.
11. If `next_action` is `ship-phase`, execute shipping inline with release-manager discipline:
   ...
   - rerun `bash scripts/dev/autoflow.sh inspect-json`
12. If `next_action` is `stop-no-candidates`, stop and report that the workflow is healthy but no active backlog candidates remain.
13. If `next_action` is `stop-blocked`, stop and report:
   - the exact blocker
